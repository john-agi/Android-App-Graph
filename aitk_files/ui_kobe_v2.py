...
def _parse_model_choice(raw: str, valid_letters: str) -> str | None:
    ...
    if re.search(r"\bNONE\b", answer):
        return "NONE"
    matches = re.findall(r"\b([A-Z])\b", answer)
...
def _parse_decide_output(raw: str) -> dict | None:
    ...
    if think_end != -1:
        ...
        try:
            ...
            parsed = _extract_json_object(post_think)
...
def _make_no_proxy_client(cfg: dict | None) -> tuple[OpenAI, str]:
...
class UIKobeV2Translator(BaseTranslator):
    ...
    def _identify_node(
        ...
        for node_id, data in G.nodes(data=True):
            ...
            candidates.append((node_id, sim, data.get("page_description", "")))
    ...
    @staticmethod
    def _format_schema_delta(delta: dict | None) -> str:
    ...
    def _edge_effect_hint(self, edge_data: dict, index: int | None = None) -> str:
    ...
    def _build_options(self, node_id: str) -> tuple[str, list[dict]]:
    ...
    def _decide(
        ...
        options_text, options_list = self._build_options(node_id)
    ...
    def _step(self, task: str, state: dict, history: dict) -> str:
...
def register(kargs: dict) -> UIKobeV2Translator:
